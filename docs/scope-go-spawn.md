...
Gas City's session runtime via `parlay gc-spawn` — opt-in, claude-kind only (`:708-711`; claude|pi since task-svq1q — see `docs/gc-prerequisite.md`).
...
3. **gc** (`PARLAY_SPAWN_LAUNCHER=gc`, env/config only) — opt-in Gas City session runtime via
   `parlay gc-spawn`; claude-kind only (`:1452-1483` per the earlier full read; claude|pi since task-svq1q — see `docs/gc-prerequisite.md`).
...
| Identity registration (`identity --register`) | ✓ | ✓ | **Full, plus one Go-only extension** | `identitycli.go` (`registerIdentityOptions`/`registerIdentity`) forwards every field bash's call at `:1779-1787` does, `BeadID`/`GCSession`/`GCCity` included. Go additionally persists `--account` into the launch spec (task-0d6mi), which bash never registered — and only when the spawn was given an explicit `--account` (`identityAccount`), never the resolved config default, so a `parlay defaults set account` rotation still reaches unpinned agents |
...
| `gc` launcher selection | ✓ (`:1452-1483`, opt-in, claude-kind only at port time; claude|pi since task-svq1q) | ✓ (opt-in via env/config, claude-kind only at port time, matching bash's own scoping; claude|pi since task-svq1q) | **Full** | `spawnpipeline.go`, `config.go` (`resolveLauncher`), `spawnpipeline_test.go`. The resolved OAuth account token is deliberately withheld from the `gc` launcher branch — only the account *name* is forwarded — mirroring bash's own stated rationale that the gc template's `[env]` block persists to disk |
