...
| Recorded state | `~/.parlay/agents/<id>/` : `context.json` {id,name,color}, `identity.md` frontmatter (launch spec: id,name,color,cwd,model,ephemeral), `session-start` sentinel | `state/<id>.meta` (key=value): window, worktree, project, harness, model, effort, kind, mode, yolo, tasktmp (+ backend fields, +pr/pr_head, +secondmate home/projects) |
