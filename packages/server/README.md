...
| `PARLAY_PUBLIC_HOST` | *(unset)* | Rewrites `http://localhost:<port>` / `http://127.0.0.1:<port>` links in served message text to `http://<host>:<port>`, so links the captain reads off-home (phone, another machine, Tailscale) resolve. Unset ⇒ no rewrite, byte-identical to legacy behavior. `"auto"` resolves the host once from `tailscale status --json` (short node name), failing open to no-rewrite if `tailscale` is unavailable. Any other value is used as the host literally (bare hostname, FQDN, or IP). Presentation-only: the stored message log is never mutated. See `src/link-rewrite.ts`. Also read by `packages/go-server` with identical semantics. |
