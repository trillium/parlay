...
const HELP: Record<string, string> = {
  ...
  alert: `parlay alert — alert pollers + agents. Immediate broadcast by default; --stagger spreads delivery to avoid a fleet-wide API spike (task-j870).\nUsage: parlay alert [--agent <id>] [--stagger [--interval <sec>] [--batch <n>]] <text...>\n  --agent <id>      Deliver only to one agent channel (immediate; nothing to stagger)\n  --stagger         Spread delivery across agents over time instead of all-at-once\n  --interval <sec>  Seconds between staggered batches (default 1.5, ±25% jitter)\n  --batch <n>       Agents delivered per batch (default 1)\nWithout --stagger, delivery is an immediate broadcast to everyone (unchanged — for urgent alerts).`,
