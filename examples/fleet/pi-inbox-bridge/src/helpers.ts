...
export function isInboxPoke(message: ChatLine, pokePrefix = "INBOX_POKE"): boolean {
	return message.role === "user" && message.text.trim().startsWith(`${pokePrefix} v1:`);
}

export type InboxConnectArgs = {
	store?: string;
	server?: string;
	error?: string;
};

const USAGE = "Usage: /inbox-connect [store] [server=http://host:port]";

/**
 * Parse `/inbox-connect` arguments. A single bare token names the store
 * (`/inbox-connect sandbox`); `server=<url>` pins the Parlay server.
 * Anything else is a usage error — pure so it can be unit-tested.
 */
export function parseInboxConnectArgs(input: string): InboxConnectArgs {
	let store: string | undefined;
	let server: string | undefined;
	for (const token of input.trim().split(/\s+/).filter((t) => t.length > 0)) {
		if (token.startsWith("server=")) {
			const value = token.slice("server=".length).replace(/\/$/, "");
			if (!value) return { error: `${USAGE} (empty server=)` };
			let protocol: string;
			try {
				protocol = new URL(value).protocol;
			} catch {
				return { error: `${USAGE} (bad server URL)` };
			}
			if (protocol !== "http:" && protocol !== "https:") {
				return { error: `${USAGE} (use http:// or https://)` };
			}
			server = value;
			continue;
		}
		if (!/^[a-z][a-z0-9_]*$/.test(token)) {
			return { error: `${USAGE} (bad store ${JSON.stringify(token)})` };
		}
		if (store !== undefined) {
			return { error: `${USAGE} (one store only)` };
		}
		store = token;
	}
	return { store, server };
