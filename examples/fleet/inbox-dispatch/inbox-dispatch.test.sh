...
#
# Run: examples/fleet/inbox-dispatch/inbox-dispatch.test.sh
set -euo pipefail
...
if grep -Fxq -- 'INBOX_POKE v1: new inbox work may be available. Run the serial inbox worker (inbox); claim and process exactly one next available item, then continue until the inbox is exhausted.' "$CAPTURE"; then
