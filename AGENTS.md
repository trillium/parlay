...
- **`tools/parlay-bin` is a dead, behind-schedule partial Go port of `bin/parlay-spawn`** — never installed on PATH, missing most of bash's feature surface. Its reconciliation scope, gap matrix, and staged plan now live at `docs/scope-go-spawn.md`. → [notes](docs/agent-notes/tools-parlay-bin-is-a-dead-behind-go-port.md)
