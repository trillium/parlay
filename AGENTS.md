...
terminal/process launch (herdr Step 5) and the post-launch liveness check
(Step 6) branch on it.

**Why this exists**: herdr has a known SIGKILL failure mode in headless/
no-WindowServer environments. The `gascity` path launches a detached
...
Shelling out to gascity's own `gc` CLI was evaluated and rejected too: it's
a full city-orchestration tool requiring a `city.toml`, a dolt DB, and k8s
client wiring, and it doesn't even build in this environment (a CGO dolt
dependency needs a system ICU regex header this box doesn't have). So
`tools/parlay-bin/gascity_spawn.go` is a from-scratch port of just the
lifecycle semantics (detached `sh -c` child, process-group signaling,
SIGTERM-then-SIGKILL stop) — see that file's own header comment for the
full rationale, including one further deliberate departure: gascity's real
provider tracks liveness via a unix control socket that requires the
creating process to stay alive, which doesn't fit `gascity-spawn`/
`gascity-stop` being separate one-shot CLI invocations with no supervisor
in between — a plain PID file does the same cross-process liveness job with
no persistent listener required.

**The three new `tools/parlay-bin` subcommands** (`gascity-spawn <agent-id>
<command> <workdir> [--state-dir DIR] [--env K=V ...] [--worktree-path P]`,
`gascity-stop <agent-id> [--state-dir DIR]`, `gascity-ping <agent-id>
...
passed to `gascity-spawn` as `--worktree-path` only when it came from
...
worktree here would make `gascity-stop` call `treehouse return` on a path
treehouse never leased. `gascity-spawn` writes the path into a
`treehouse-path` sidecar in the state dir; `gascity-stop` reads it and, if
`treehouse` is on PATH, runs `treehouse return <path>` **before** signalling
the process — always with `cmd.Dir` set to the leased path itself, since
`treehouse` resolves its pool from the process cwd, not a flag (robots-d04t,
documented above). Every step here is best-effort: a missing sidecar, a
missing `treehouse` binary, or a failing `treehouse return` never blocks the
...
`agent start` argv). The gascity path has no pane to paste into, so the
...
on the gascity path for a real unattended agent.

**Post-launch liveness** also has no herdr equivalent to poll (`agent wait`
needs a herdr-registered agent). Step 6's gascity branch instead polls `GET
$PARLAY/api/chat/subscribers` for an entry whose `channel` equals the agent
id, for up to `PARLAY_SPAWN_LIVENESS_TIMEOUT_MS` (default 60000, same env
var as the herdr watchdog), backgrounded/disowned the same way so batch
...
`gascity-ping` for manual inspection.

Tests: `tools/parlay-bin/gascity_spawn_test.go` covers the start/stop/ping
lifecycle, duplicate-session refusal, idempotent/stale-pid stop, SIGKILL
escalation (via the `stopGrace` var, shrunk in-test), and the treehouse
