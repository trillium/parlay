...
- **The Go spawner is `tools/cli/internal/spawn`, in-process behind `parlay spawn`** — `tools/parlay-bin` is gone (folded in, task-42qot), so there is no second spawner binary and no PATH resolution order. `PARLAY_SPAWN_IMPL=bash` (or `spawnImpl = "bash"` in `~/.parlay/config.toml`) is the only way back to `bin/parlay-spawn`; its reconciliation scope and gap matrix are `docs/scope-go-spawn.md`. → [notes](docs/agent-notes/go-spawner-folded-into-tools-cli.md)
