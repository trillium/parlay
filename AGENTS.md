...
- **`tools/parlay-bin` is a dead, behind-schedule partial Go port of `bin/parlay-spawn`** — never installed on PATH, missing most of bash's feature surface, and `docs/scope-go-spawn.md` it cites doesn't exist either. Don't assume it works or is current. → [notes](docs/agent-notes/tools-parlay-bin-is-a-dead-behind-go-port.md)
