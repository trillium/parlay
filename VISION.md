...
Agent status is a first-class relay concept: crew state and lifecycle live in the relay, backed by a Gas City bead store (`GC_BEADS`) at a parlay-controlled path; no PAI federation is required.
...
The Go implementation replaced the TypeScript CLI; that CLI and the diff harness that proved the port faithful are both gone, so parity is a historical fact rather than a maintained property.
Every verb is a Go verb; no verb acquires a TypeScript counterpart.
