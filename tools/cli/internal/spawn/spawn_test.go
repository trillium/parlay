...
type mockLauncher struct {
	...
	failStart           bool
	busyStarts          int // first N AgentStart calls report agent_pane_busy (robots-i4pi retry tests)
...
func (m *mockLauncher) AgentStart(opts AgentStartOptions) (string, error) {
	...
	if m.busyStarts > 0 {
		m.busyStarts--
		return `{"error":"agent_pane_busy: pane is not an available shell"}`, &mockErr{"exit status 1"}
	}
	if m.failStart {
...
// The PARLAY_SPAWN_VIA_CLI handshake is gone (task-42qot): spawn runs
// in-process inside `parlay`, so there is no cross-binary entry left to
// police. A bare call must proceed straight to the real gates — here, the
// model gate, proving the pipeline actually ran.
func TestRunSpawnCommandNeedsNoHandshake(t *testing.T) {
	...
	if strings.Contains(out, "refusing to run directly") {
		t.Errorf("no handshake refusal should exist anymore; output:\n%s", out)
	}
	if !strings.Contains(out, "no model was chosen") {
