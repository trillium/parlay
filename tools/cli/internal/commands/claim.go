	...
	if dod == "" {
		...
		if claimStoreForID(task.ID) == "robots" {
			dod = fmt.Sprintf("Fix it, then run 'robots close %s' and report the outcome with 'reply'.\n\n"+
				...
				"- Only claim FIXED when the fix has actually LANDED, and prove it with `parlay landed <n>` (exit 0 = landed; anything else = not). It asserts both halves at once: the PR is MERGED *in this checkout's own repository*, and the merge commit is reachable from the remote base branch. Do NOT hand-run `gh pr view <n> --json state,mergedAt` for this: a bare gh command resolves to `upstream`, not `origin`, so in a fork checkout it answers about a stranger's same-numbered PR — observed reporting MERGED months earlier while the real PR was still OPEN (robots-0a77). An OPEN or check-failing PR is NOT fixed: signal needs-decision or blocked, never done.\n"+
				"- That proof assumes origin/main is the thing that actually RUNS. Where the working tree is itself the deployment target it is not — in pai-hooks, `~/.claude/hooks` symlinks into the checkout, so local main is live and origin has been 20 commits behind it. `parlay merge-gate` prints ORIGIN LAGS LIVE when it sees that drift. When it does, LANDED means the DEPLOYED branch contains the commit, not merely origin/main: reconcile the two before reporting done, or say plainly that the fix is merged but NOT live — never convert 'merged' into 'fixed' on its own (robots-oex0).\n"+
				...
				"- A 5 can also mean your own push has not reached origin yet (`head-not-pushed`, robots-bn5d). Every verdict is about ORIGIN's head — `git push no-mistakes` goes to the mirror, and the pipeline pushes to origin asynchronously, so merging before it lands the OLD head and silently DROPS the fix you just wrote. Confirm `gh pr view <n> --json headRefOid` matches your local HEAD before merging, and re-run the gate after the push arrives: a new head restarts the review, so a 0 can legitimately become 3, 4 or 5 afterwards.\n"+
				"- NEVER switch or leave a primary/shared checkout on a feature branch — that strands whatever session sits there. Do all repo work in an isolated worktree (fm-spawn crew, or `git worktree add`), and leave the primary on its original branch.\n"+
				"- A worktree does NOT isolate a symlinked subtree: `git worktree add` copies the symlink, not the tree behind it, so writes under it land in the SHARED checkout it points at. Worktree the repo that actually owns the files — `git -C \"$(git -C <dir> rev-parse --show-toplevel)\" worktree add …` after resolving the path. Concretely: to change anything under `~/.claude/hooks`, worktree `~/code/pai-hooks` (use its `scripts/hooks-worktree`), NOT `~/.claude` — robots-1186. Check before you start: `ls -ld <the-dir-you-will-edit>`.", task.ID)
		} else {
