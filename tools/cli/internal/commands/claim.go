	...
	if dod == "" {
		...
		if claimStoreForID(task.ID) == "robots" {
			dod = fmt.Sprintf("Fix it, then run 'robots close %s' and report the outcome with 'reply'.\n\n"+
				...
				"- Obey the gate's exit code, not just its sign. 3 = blocked on the CODE: fix it on the branch. 4 = NEEDS-DECISION, the reviewer itself is unavailable and no work on the branch will change that — do NOT poll and do NOT merge anyway. Signal `parlay status needs-decision` with the gate's one-line reason and stop; the captain chooses merge-and-disclose or park. An unbounded wait on a rate-limited reviewer is itself a defect (robots-8kkq).\n"+
