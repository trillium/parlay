...
func routePIIModel(pii piiState, kind, model string) (newKind, newModel string) {
	...
	if pick == "" {
		pick = live[0]
		fmt.Fprintf(os.Stderr, "parlay-spawn: no-pii — none of the preferred free models are offered any more; falling back to %s. Update piiFreeModelPreference in tools/parlay-bin/pii.go.\n", pick)
	}
