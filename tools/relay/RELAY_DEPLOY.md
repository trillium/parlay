...
The relay is a single per-user process. Its canonical runtime is
`$TMPDIR/parlay`, and all monitors use that same control socket. The CLI owns
upstream server resolution before it starts a monitor.
...
1. Returns immediately if the canonical relay already answers `/health` (no
   double-start).
